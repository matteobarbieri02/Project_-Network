...
```
```{r}
prova_merge <- subset(prova_merge)
```

```{r}
df_clu1 <- prova_merge[prova_merge$cluster==1, ]
A <- df_clu1[,-1]
head(A)
```

```{r}
df_clu2 <- prova_merge[prova_merge$cluster==2, ]
B <- df_clu2[,-1]
head(B)
```

```{r}
g_cl1 <- graph_from_data_frame(A, directed = FALSE)
...
```{r}
g_cl2 <- graph_from_data_frame(B, directed = FALSE)
